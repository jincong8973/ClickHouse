...
$CLICKHOUSE_CLIENT -q 'SELECT count(), sum(total_replicas), sum(length(active_replicas) FROM system.replicas WHERE database=currentDatabase()'
$CLICKHOUSE_CLIENT -q 'SELECT count(), sum(total_replicas), sum(length(active_replicas) FROM system.replicas WHERE database=currentDatabase() SETTINGS max_block_size=1, send_logs_level=\'test\
$CLICKHOUSE_CLIENT -q 'SELECT count(), sum(total_replicas), sum(length(active_replicas) FROM system.replicas WHERE database=currentDatabase() SETTINGS max_block_size=7'
$CLICKHOUSE_CLIENT -q 'SELECT count(), sum(total_replicas), sum(length(active_replicas) FROM system.replicas WHERE database=currentDatabase() SETTINGS max_block_size=111'
